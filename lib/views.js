...
const { MAX_DESTINATIONS } = require('./destinations');
const { canViewResults, resultsHiddenReason } = require('./policy');
const { getAllVoters } = require('./poll');
const {
...
function headerText(text) {
  const s = String(text ?? '');
  return s.length <= SLACK_HEADER_LIMIT ? s : s.slice(0, SLACK_HEADER_LIMIT - 1) + '…';
}
...
function buildVoteModal(poll, previousVotes = {}) {
  ...
  return {
    ...
    private_metadata: JSON.stringify({ pollId: poll.id }),
    blocks: [
      { type: 'header', text: { type: 'plain_text', text: headerText(poll.title) } },
...
function buildPollBlocks(poll) {
  ...
  const frame = questionBlocks => [
    { type: 'header', text: { type: 'plain_text', text: headerText(`📊  ${poll.title}`), emoji: true } },
    ...(poll.description ? [{ type: 'section', text: { type: 'mrkdwn', text: poll.description } }] : []),
