...
// Slack's own figures: "up to 50 blocks in each message, and 100 blocks in
// modals or Home tabs." Over either and the whole thing is refused. Messages
// were capped at 100 here as well, so a poll of seven or eight questions built
// a message Slack would not post - and with a block per option, that is not a
// big poll. See fitQuestionBlocks for what gives way instead.
...
// One question as one block of text: its options, and their tallies where this
// viewer may see them - without the voter names, which are what make the full
// layout long. Used when that layout, a block per option, will not fit.
function condensedQuestionText(q, qi, poll, viewerId = null, interactive = false) {
  const qVotes = pollVotes(poll)[qi] || {};
  const visible = canViewResults(poll, viewerId);
  const lines = [`${questionTypeIcon(q)}  *${qi + 1}. ${questionText(q)}*`];
  const count = list => (Array.isArray(list) ? list.length : 0);

  if (q.type === 'open_ended') {
    const n = Object.keys(qVotes).length;
    lines.push(visible ? `_${n} response${n !== 1 ? 's' : ''}_` : `_${resultsHiddenReason(poll)}_`);
  } else if (q.type === 'likert') {
    (q.options || []).forEach((stmt, si) => {
      const ratings = qVotes[si] || {};
      const total = Object.values(ratings).reduce((s, v) => s + count(v), 0);
      const score = Object.entries(ratings).reduce((s, [r, v]) => s + (Number(r) + 1) * count(v), 0);
      lines.push(visible && total ? `•  ${stmt}  -  *${(score / total).toFixed(1)}*/5  (${total})` : `•  ${stmt}`);
    });
    if (!visible) lines.push(`_${resultsHiddenReason(poll)}_`);
  } else if (q.type === 'ranking') {
    const rankings = Object.values(qVotes);
    const avg = (q.options || []).map((_, oi) => {
      const ranks = rankings.map(r => parseInt(String(r || '').split(',')[oi], 10)).filter(n => n > 0);
      return ranks.length ? ranks.reduce((a, b) => a + b, 0) / ranks.length : null;
    });
    const order = (q.options || []).map((opt, oi) => ({ opt, avg: avg[oi] }));
    if (visible && rankings.length) order.sort((a, b) => (a.avg ?? 999) - (b.avg ?? 999));
    order.forEach(({ opt, avg: a }, i) => {
      lines.push(visible && rankings.length ? `${i + 1}.  ${opt}${a !== null ? `  -  avg *${a.toFixed(1)}*` : ''}` : `•  ${opt}`);
    });
    if (!visible) lines.push(`_${resultsHiddenReason(poll)}_`);
  } else {
    const total = Object.values(qVotes).reduce((s, v) => s + count(v), 0);
    (q.options || []).forEach((opt, oi) => {
      const n = count(qVotes[oi]);
      const tally = visible && total
        ? `  \`${pollProgressBar(n, total, 8)}\`  ${Math.round((n / total) * 100)}%  (${n})`
        : '';
      lines.push(`${OPTION_EMOJIS[oi] || `${oi + 1}.`}  ${opt}${tally}`);
    });
    lines.push(visible ? (total ? '' : '_No votes yet_') : `_${resultsHiddenReason(poll)}_`);
  }
  if (interactive && !isInlineVotable(q.type)) lines.push('_Answer this one with 🗳️ Vote._');
  return clampText(lines.filter(Boolean).join('\n'));
}

// The same question as a block, with a dropdown to vote from where a question
// can be answered in one pick. A dropdown is one element where the full layout
// spends a block per option, which is the whole saving.
function condensedQuestionBlock(q, qi, poll, viewerId = null, interactive = false) {
  const block = { type: 'section', text: { type: 'mrkdwn', text: condensedQuestionText(q, qi, poll, viewerId, interactive) } };
  if (interactive && isInlineVotable(q.type) && (q.options || []).length) {
    block.accessory = {
      type: 'static_select',
      // Unique per question, for the same reason the buttons are unique per
      // option: Slack rejects a repeated action_id within one message.
      action_id: `vote_select_${qi}`,
      placeholder: { type: 'plain_text', text: q.allowMultiple ? 'Add or take back…' : 'Vote…' },
      options: q.options.map((opt, oi) => ({
        text: { type: 'plain_text', text: optionText(`${oi + 1}. ${opt}`) },
        value: `${poll.id}::${qi}::${oi}`
      }))
    };
  }
...
// The questions of a poll in at most `budget` blocks, in the richest layout
// that fits:
//
//   full       a block per option, a button on each - the usual case
//   condensed  a block per question, a dropdown on each
//   packed     several questions to a block, text only
//
// Nothing is cut until even packing will not fit, which takes a poll far past
// anything real (every question near the length caps). Only then does the
// tail go, and it is counted, so the reader knows it is there - trimming a
// ballot quietly would drop questions nobody could tell they had missed.
function fitQuestionBlocks(poll, { viewerId = null, interactive = false, budget }) {
  const questions = poll.questions || [];
  const full = questions.flatMap((q, qi) => buildQuestionResultBlock(q, qi, poll, viewerId, interactive));
  if (full.length <= budget) return full;

  const condensed = questions.map((q, qi) => condensedQuestionBlock(q, qi, poll, viewerId, interactive));
  const divided = condensed.flatMap(b => [b, { type: 'divider' }]);
  if (divided.length <= budget) return divided;
  if (condensed.length <= budget) return condensed;

  const sections = [];
  let text = '', held = 0;
  questions.forEach((q, qi) => {
    const t = condensedQuestionText(q, qi, poll, viewerId, false);
    if (text && text.length + 2 + t.length > 2900) {
      sections.push({ text, held });
      text = t; held = 1;
    } else {
      text = text ? `${text}\n\n${t}` : t; held++;
    }
  });
  if (text) sections.push({ text, held });

  const tail = interactive ? 'Too long to vote on from the message - press *🗳️ Vote* to answer it.' : null;
  const room = budget - (tail ? 1 : 0);
  let kept = sections;
  let note = tail;
  if (sections.length > room) {
    kept = sections.slice(0, room - (tail ? 0 : 1));
    const left = questions.length - kept.reduce((s, x) => s + x.held, 0);
    note = `…and ${left} more question${left !== 1 ? 's' : ''}.${tail ? ` ${tail}` : ''}`;
  }
  return [
    ...kept.map(s => ({ type: 'section', text: { type: 'mrkdwn', text: s.text } })),
    ...(note ? [{ type: 'context', elements: [{ type: 'mrkdwn', text: `_${note}_` }] }] : [])
  ];
}
...
function capBlocks(blocks, limit) {
  ...
  return [
    ...
    // Only the preview is trimmed. It used to promise that all its blocks would
    // be posted, which a long poll's message - condensed to fit - does not do.
    { type: 'context', elements: [{ type: 'mrkdwn', text: '_…preview trimmed. Every question is still in the poll._' }] }
  ];
}

// Whether a poll's ballot fits in one modal. Every Likert statement and every
// ranked item is a field of its own, so a poll well inside the question and
// option caps can still need more than Slack's 100 blocks - and then the Vote
// button opens nothing, on a poll that posted perfectly well. Checked while the
// poll is being written, when a question can still be cut.
function ballotFits({ questions = [], title = '', description = '' } = {}) {
  const probe = { id: 'probe', title, description, questions, votes: {}, anonymous: false };
  return buildVoteModal(probe).blocks.length <= MAX_VIEW_BLOCKS;
}

const BALLOT_FULL = "This poll is too long for Slack's voting form, which holds 100 fields - and every Likert statement and ranked item is one. Remove a question or some statements, or split it into two polls.";

// `locked` is the questions this person answered on a poll that does not allow
// vote changes. They are listed as done rather than asked again, so the modal
// can finish a poll someone started from the message's own buttons.
function buildVoteModal(poll, previousVotes = {}, { locked = new Set() } = {}) {
  ...
  const questionBlocks = poll.questions.flatMap((q, qi) => {
    ...
    if (locked.has(qi)) {
      return [{
        type: 'section',
        text: { type: 'mrkdwn', text: clampText(`✅  *${qi + 1}. ${questionText(q)}*\n_Already answered - vote changes are off for this poll._`) }
      }];
    }
    ...
    if (q.type === 'likert') {
      ...
      return [
        ...
        ...q.options.map((stmt, si) => {
          const rated = likertOpts.find(o => o.value === String(prev.ratings?.[si]));
          return {
            type: 'input',
    ...
    if (q.type === 'ranking') {
      ...
      return [
        ...
        ...q.options.map((opt, oi) => {
          // '0' is what an unranked item was stored as, not a rank.
          const ranked = rankOpts.find(o => o.value === String(prev.ranks?.[oi]));
          return {
            type: 'input',
...
function buildPollBlocks(poll) {
  ...
  return frame(fitQuestionBlocks(poll, { interactive: !isClosed, budget: MAX_MESSAGE_BLOCKS - frame([]).length }));
}
...
// A message - the final results on close, or /poll-share - so it has the
// message limit, and had no limit applied at all.
function buildResultsBlocks(poll, heading, viewerId = null) {
  ...
  return [...head, ...fitQuestionBlocks(poll, { viewerId, budget: MAX_MESSAGE_BLOCKS - head.length })];
}

// `note` is for what the voter needs to know about how their vote was taken -
// today, only that part of the form was overruled by an earlier answer.
function buildPostVoteModal(poll, viewerId = null, { note = null } = {}) {
  const participants = getAllVoters(poll).size;
  const head = [
    { type: 'section', text: { type: 'mrkdwn', text: `Your vote has been recorded for *${pollDisplayTitle(poll)}*!` } },
    ...(note ? [{ type: 'section', text: { type: 'mrkdwn', text: `⚠️ ${clampText(note)}` } }] : []),
  ...
  return {
    ...
    close: { type: 'plain_text', text: 'Close' },
    // Uncapped, a long poll built a view Slack refused - after the vote had
    // been saved, so the voter was told nothing about a vote that counted.
    blocks: [...head, ...fitQuestionBlocks(poll, { viewerId, budget: MAX_VIEW_BLOCKS - head.length })]
  };
}
...
module.exports = {
  ...
  clampText,
  MAX_VIEW_BLOCKS,
  condensedQuestionText,
  condensedQuestionBlock,
  fitQuestionBlocks,
  capBlocks,
  buildVoteModal,
  ballotFits,
