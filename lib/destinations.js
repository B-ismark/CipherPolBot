...
const {
  claimNotification, releaseNotification, notificationLimitMessage
} = require('./validation');
...
async function resolveDestinations(client, { channelIds, userIds }, fallbackChannelId, actorId) {
  ...
  for (const uid of users) {
    ...
    if (!isSelf && !claimNotification(uid)) {
      failures.push({ label: `<@${uid}>`, reason: notificationLimitMessage() });
      continue;
    }
    // claimed says whose slot has to be handed back if this DM never lands -
    // here if Slack will not open the conversation, and in postPollTo if the
    // message itself is refused. Null for your own copy, which is never charged.
    const claimed = isSelf ? null : uid;
    try {
      const r = await client.conversations.open({ users: uid });
      targets.push({ channel: r.channel.id, label: `<@${uid}>`, claimed });
    } catch (e) {
      if (claimed) releaseNotification(claimed);
      failures.push({ label: `<@${uid}>`, reason: e.data?.error || e.message });
    }
  }

  // A target dropped as a duplicate is never posted to, so its claim goes back.
  // Unreachable today - the conversation picker offers no group DMs, so a D
  // channel can only come from a person, and people are deduped by id before
  // any claim is made - but a leaked claim is invisible and lasts until the
  // process restarts, which is the worst shape a limiter fault can take.
  const kept = dedupeTargets(targets);
  for (const t of targets) {
    if (t.claimed && !kept.includes(t)) releaseNotification(t.claimed);
  }
  return { targets: kept, failures, redirected, usedFallback };
}
...
async function postPollTo(client, { text, blocks }, targets) {
  ...
  results.forEach((r, i) => {
    if (r.status === 'fulfilled') {
      ...
      // The claim made in resolveDestinations stands: the DM arrived, so the
      // recipient's slot is spent.
    } else {
      // It did not arrive, so it costs them nothing. A slot spent on a DM that
      // never went out was what locked people out of a feature that had never
      // delivered them anything, five failures at a time.
      if (targets[i].claimed) releaseNotification(targets[i].claimed);
