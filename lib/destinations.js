...
function describeFailures(failures) {
  ...
  const needsScope  = failures.some(f => /missing_scope|not_allowed_token_type/.test(`${f.reason}`));
  return `Could not post to ${list}.`
    + (needsInvite ? ' For a private channel, invite me to it first (`/invite @Cipher Pol`).' : '')
    + (needsScope ? ' A DM needs the `im:write` scope - if the app was installed before that was added, it has to be reinstalled to pick it up.' : '');
}
...
module.exports = {
  ...
  resolveChannelInfo, resolveDestinations, postPollTo, reachesCreator, describeFailures, toMessageRefs
};
