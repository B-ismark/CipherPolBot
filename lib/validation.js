...
// A backstop, not the limit that bites. A poll being built travels inside a
// Slack view's private_metadata (see MAX_VIEW_METADATA), so what a poll may
// hold is really measured in bytes rather than questions - roughly 16 typical
// multiple-choice questions, or 7 Likert matrices of eight statements each. No
// fixed count can describe that, because a 7-question Likert poll and a
// 16-question multiple-choice poll are the same size. This number therefore
// cannot be reached through the poll builder; it stays so that a poll assembled
// any other way still has a ceiling.
const MAX_QUESTIONS_PER_POLL = 50;
const MAX_OPTIONS_PER_QUESTION = 10;
// Slack rejects a view whose private_metadata runs past 3000 characters, and a
// half-finished poll travels entirely inside it. A rejected view is a silent
// failure - the worst kind - so the draft is measured before it is carried and
// the ceiling is reported in words instead. This is the limit that actually
// fires; MAX_QUESTIONS_PER_POLL above is the backstop behind it.
const MAX_VIEW_METADATA = 3000;

const MAX_POLLS_PER_USER_PER_DAY = 10;
const MAX_NOTIFY_SUBSCRIBERS_PER_POLL = 20;
// A courtesy cap on how many poll DMs one person can receive in an hour, not a
// security control - the controls are on the sending side, where the abuse is:
// ten polls a day per creator, thirty share destinations an hour per sender.
// This was 5, which is fewer than it takes to test a poll: a creator sending
// themselves a few drafts hit it, and a hit reads exactly like a broken
// feature, because what you get is no DM.
const MAX_NOTIFICATIONS_PER_USER_PER_HOUR = 15;
...
// Whether a draft poll still fits in the Slack view that has to carry it.
// Checked wherever the draft grows or is handed to another screen, so the
// builder refuses in words rather than letting Slack reject the view in silence.
function draftFitsInView(meta) {
  return JSON.stringify(meta).length <= MAX_VIEW_METADATA;
}
...
// One more DM to this person, taken now and given back if it does not arrive.
//
// Two faults meet here, and only a claim settles both.
//
// A single call that checked and charged at once - the version this replaced -
// spent the slot at the moment of asking, so a DM that then failed to open or
// failed to post still cost the recipient one. Five failures and they were
// locked out of a feature that had never delivered them anything: a poll that
// never arrives, made permanent by retrying.
//
// Splitting it into a check and a later charge fixed that and opened a race:
// with one slot left, two sends to the same person both passed the check
// before either charged, and the cap was quietly exceeded. Claiming is
// atomic - the slot is gone the instant it is granted - and releaseNotification
// hands it back on the paths that fail, which is what keeps an undelivered DM
// free without letting two of them share a slot.
function claimNotification(userId) {
  if (!canNotify(userId)) return false;
  notificationTracker[userId] = notificationTracker[userId] || [];
  notificationTracker[userId].push({ timestamp: Date.now() });
  return true;
}

// Give back a claim whose DM never went out. Pops the newest entry, which
// inside one send is the claim just made.
function releaseNotification(userId) {
  const claims = notificationTracker[userId];
  if (claims?.length) claims.pop();
}

// A read-only look at the budget, for a caller that only wants to know.
function canNotify(userId) {
  return withinLimit(notificationTracker, userId, MAX_NOTIFICATIONS_PER_USER_PER_HOUR, HOUR_MS);
}

function notificationLimitMessage() {
  return `has had ${MAX_NOTIFICATIONS_PER_USER_PER_HOUR} poll DMs from me in the past hour, which is the cap - it clears on its own`;
}
