...
// Whether one more DM to this person is allowed, without spending the budget on
// asking - the same door-and-till split canCreatePoll already uses, and for a
// sharper reason here. The single call this replaced spent the slot up front, so
// a DM that then failed to open or failed to post still cost the recipient one.
// Five such failures and they were locked out of a feature that had never
// delivered them anything: the symptom was a poll that never arrives, and
// retrying made it permanent. Both callers now check here and charge only after
// Slack has accepted the message.
function canNotify(userId) {
  return withinLimit(notificationTracker, userId, MAX_NOTIFICATIONS_PER_USER_PER_HOUR, HOUR_MS);
}

// Called only once a DM has actually gone out.
function spendNotification(userId) {
